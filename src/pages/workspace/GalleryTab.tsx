...
export function GalleryTab({ projectId }: { projectId: string }) {
  ...
  const load = useCallback(async () => {
    const { data: dp } = await supabase.from('daily_progress').select('id').eq('project_id', projectId);
    if (!dp || dp.length === 0) return;
    const ids = dp.map(d => d.id);
    const { data: ph } = await supabase.from('daily_photos').select('*').in('daily_id', ids).order('taken_at', { ascending: false });
    setPhotos(ph || []);
  }, [projectId]);
  ...
  if (photos.length === 0) return <EmptyState message="Belum ada foto. Upload progress foto dari tab Timeline." />;

  return (
    <div>
      <h3 style={{ fontSize: 16, marginBottom: 16 }}>Gallery</h3>
      {Object.entries(grouped).map(([date, items]) => (
        <div key={date} style={{ marginBottom: 24 }}>
          <h4 style={{ fontSize: 14, marginBottom: 8, color: 'var(--text-muted)' }}>
